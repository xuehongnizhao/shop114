 ...
 */
@property (strong, readwrite, nonatomic) SkyNumberButton *leftButton;

/**
 *  Right function button
 */
@property (strong, readwrite, nonatomic) SkyNumberButton *clearButton;

/**
 *  APNumberPad delegate
 */
@property (weak, readwrite, nonatomic) id<SkyKeyboardDelegate> delegate;
 ...
 */
@property (strong, readwrite, nonatomic) Class<SkyKeyboardStyle> styleClass;

@end

@implementation SkyKeyboard
...
{
    return [self keyboardWithDelegate:delegate keyboardStyleClass:nil];
...
- (instancetype)initWithDelegate:(id<SkyKeyboardDelegate>)delegate numberPadStyleClass:(Class)styleClass {
    ...
    if (self) {
        ...
        for (int i = 0; i < 11; i++) {
            SkyNumberButton *numberButton = [self numberButton:i];
...
- (void)layoutSubviews {
    ...
    for (int i = 1; i < self.numberButtons.count - 1; i++) {
        SkyNumberButton *numberButton = self.numberButtons[i];
    ...
    //
    left += buttonSize.width + sep;
    self.leftButton.frame = CGRectMake(left, top, 2*buttonSize.width, buttonSize.height);
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    for (SkyNumberButton *b in self.numberButtons) {
        if (CGRectContainsPoint(b.frame, location)) {
            b.highlighted = YES;
        } else {
            b.highlighted = NO;
            [b distinguish_touchesCancelled:touches withEvent:event];
        }
    }
}
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (!CGRectContainsPoint(self.clearButton.frame, location)) {
        [self.clearButton distinguish_touchesCancelled:touches withEvent:event];
    ...
    if (!CGRectContainsPoint(self.leftButton.frame, location)) {
        [self.leftButton distinguish_touchesCancelled:touches withEvent:event];
    }
...
- (void)setStyleClass:(Class)styleClass {
    if (styleClass) {
        ...
        _styleClass = [SkyKeyboardDefaultStyle class];
    }
}
...
- (void)functionButtonAction:(id)sender {
    ...
    if ([self.delegate respondsToSelector:@selector(keyboard:functionButtonAction:textInput:)]) {
        [self.delegate keyboard:self functionButtonAction:sender textInput:self.textInput];
    }
}
...
- (SkyNumberButton *)numberButton:(int)number {
    SkyNumberButton *b = [SkyNumberButton buttonWithBackgroundColor:[self.styleClass numberButtonBackgroundColor]
...
- (SkyNumberButton *)functionButton {
    SkyNumberButton *b = [SkyNumberButton buttonWithBackgroundColor:[self.styleClass functionButtonBackgroundColor]
