...
- (CAShapeLayer *)createIndicatorWithColor:(UIColor *)color andPosition:(CGPoint)point {
    ...
    [path addLineToPoint:CGPointMake(8, 3)];
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
     ...
     *  it's my custom
     */
    BOOL isShowTableView=YES;
    if ([self.delegate respondsToSelector:@selector(menu:didSelectMenuAtIndexPath:)])
    {
      isShowTableView =[self.delegate menu:self didSelectMenuAtIndexPath:tapIndex];
    }
    
    [self shouldShowMenuTableview:isShowTableView atTapIndex:tapIndex];
    
 
}

-(void)shouldShowMenuTableview:(BOOL) isshowTableview atTapIndex:(NSInteger) tapIndex
{
    if (isshowTableview==NO) return;
    ...
    if (tapIndex == _currentSelectedMenudIndex && _show) {
        ...
        [self setMenuTitleColor:[UIColor blackColor] atIndexPath:tapIndex];
    } else {
        _currentSelectedMenudIndex = tapIndex;
        ...
        [self setMenuTitleColor:self.selectColor atIndexPath:tapIndex];
...
{
    CATextLayer *titleLayer = (CATextLayer *)_titles[indexPath.column];
...
-(void)setMenuTitleColor:(UIColor *)color atIndexPath:(NSInteger)index
{
    ...
    CATextLayer *titleLayer = (CATextLayer *)_titles[index];
